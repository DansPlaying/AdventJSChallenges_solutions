...
function countHours(year, holidays) {
	...
	holidays.forEach((element) => {
		...
		const holyDay = new Date(
			year,
			parseInt(element.slice(0, 2)) - 1,
			parseInt(element.slice(3))
		)
		let dayOfWeekHolyDay = holyDay.getDay()
		console.log(dayOfWeekHolyDay)
		if(dayOfWeekHolyDay != 0 && dayOfWeekHolyDay != 6)
		  extraHours += 2
	})
